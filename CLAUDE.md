...
「亞太數學奧林匹亞競賽」（正式競賽，APMO）的 content 目錄名稱是 `APMO`（網址用英文簡稱，方便輸入；顯示名稱仍是中文，由 `content/APMO/_index.md` 的 `title` front matter 控制）。PDF 靜態檔案路徑維持中文，放在 `static/files/亞太數學奧林匹亞競賽/...`。注意此與「亞太數學奧林匹亞競賽初選考試」（初選，目錄用中文全名）為不同競賽，勿混淆。
