...
「亞太數學奧林匹亞競賽初選考試」的 content 目錄名稱是 `APMO`（網址用英文，方便輸入；顯示名稱仍是中文，設定在 `content/APMO/_index.md` 的 `title` front matter，不隨目錄名稱變動）。PDF 靜態檔案路徑維持中文，仍放在 `static/files/亞太數學奧林匹亞競賽初選考試/...`，兩者名稱不需一致。

`content/APMO/` 依各年度是否分階段而有兩種結構：

- 單一考試的年度（例如 2019、2020）：題目直接寫在 `content/APMO/<年度>/_index.md`
- 分階段的年度（例如 2022 年起分第一階段、第二階段）：每個階段是自己的子目錄，題目寫在該子目錄的 `_index.md`（`bookCollapseSection: true`）：

```
content/APMO/<年度>/第一階段/_index.md
content/APMO/<年度>/第二階段/_index.md
```
